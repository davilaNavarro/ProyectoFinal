 ...
 * @author DAvil
 */
@WebServlet(name = "RepartidoresPorFechas", urlPatterns = {"/RepartidoresPorFechas"})
public class RepartidoresPorFechas extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet RepartidoresPorFechas</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet RepartidoresPorFechas at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
...
//        processRequest(request, response);
           ...
           String id_repartidor = request.getParameter("cboRepartidor");
           request.setAttribute("comboRepar", id_repartidor);
           ...
           if(fecHasta.compareTo(fecDesde)>0){
                ...
                ArrayList <MensualesRepar> listaMenRep = MensualRep.listaMenRep(id_repartidor, fecDesde, fecHasta);
                ...
                request.getRequestDispatcher("tablaRepartidorFecha.jsp").forward(request, response);
           }else{
              errores = "Rango de fecha definido incorrecto";
              request.setAttribute("msg", errores);
              request.getRequestDispatcher("listaporRepartidorFecha.jsp").forward(request, response);
           }
